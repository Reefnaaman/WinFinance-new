    ...
    <section id="advantages" className="py-16 md:py-32 bg-muted/30">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold mb-4 text-balance">למה לבחור ב-WinFinance?</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 max-w-5xl mx-auto">
          {advantages.map((advantage, index) => (
            <Card key={index} className="p-6 md:p-8 text-center hover:shadow-xl transition-all duration-300 bg-card">
              <div className="bg-primary/10 w-16 h-16 rounded-full flex items-center justify-center mb-6 mx-auto">
                <advantage.icon className="w-8 h-8 text-primary" />
              </div>
              <h3 className="text-xl md:text-2xl font-bold mb-4">{advantage.title}</h3>
              <p className="text-base md:text-lg text-muted-foreground leading-relaxed">{advantage.description}</p>
            </Card>
          ))}
        </div>
      </div>
